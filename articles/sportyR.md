...
[`sportyR`](https://sportyr.sportsdataverse.org/) package. My aim with
this package is to provide high-quality, reliable, baseline plots to use
for geospatial analysis of sports data. I’m excited to showcase some of
...
3.  Head to
    [RStudio.com](https://posit.co/download/rstudio-desktop/ "Download the appropriate version of RStudio (Free) for your operating system to use with R")
...
`sportyR` is live on CRAN, and the most recent release can be installed
by running:
...
specifically on a sports playing surface. So that begs the question:
what sports can we plot using `sportyR`?

You’re in luck: these kinds of questions are natively answered by what
I’ve called the `cani_{question}()` family of functions. They’re
...
and that’s the exact syntax you can follow to have the package answer
those questions. Here’s an example:
...
I’ll highlight the fact that these are **case-insensitive** searches.
Ask away to your heart’s content!

There’s one other `cani_{question}()` function I’ll highlight more in a
...
interested in drawing a regulation [NBA](https://www.nba.com) basketball
court. `sportyR` seeks to make this as easy as possible:
...
![](getting-started-example-basketball-court-1.png)

Easy as that to get started. Here’s a quick overview of the arguments
(which are included for all of the `geom_{sport}()` functions):

- `league`: This is a required parameter, but `custom` is a viable value
  for any sport. As a quick note, using this `custom` option will
  require you to specify all parameters of the surface you’re looking to
  create. This is case-insensitive
