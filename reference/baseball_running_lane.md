...
The running lane is entirely in foul territory. The depth should be
measured from the foul-side edge of the baseline to the outer edge of
the running lane mark
...
- running_lane_length:
  ...
  The total distance of the running lane, from where it first starts to
  its terminus near first base

- running_lane_start_distance:

  The distance from the back tip of home plate that the running lane
  starts
...
All measurements should be given "looking down the line" (e.g. as they
