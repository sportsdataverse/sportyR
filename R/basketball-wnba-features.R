#' Generate the data frame for the points that comprise a regulation WNBA court
#'
...
wnba_feature_court_bground = function(full_surf = TRUE,
  ...
  # A regulation WNBA court is 94' long and 50' wide, but this will only draw
...
wnba_feature_center_circles = function(full_surf = TRUE,
  ...
  # A WNBA court has two circles in the center: one of radius 2' (interior), and
...
wnba_feature_free_throw_dashed_semi_circle = function(full_surf = TRUE,
  ...
  # Per the WNBA rule book, the solid portion of the circle extends along an arc
...
#' Generate the list of colors for a WNBA court plot. The defaults can be
#' overwritten by supplying the names of the list elements to the
#' \code{geom_wnba()} function (or its wrapper \code{geom_basketball()})
#'
...
#' Create a ggplot2 instance that represents a regulation WNBA court, with the
...
#'   colors to pass to the \code{wnba_features_set_colors()} function
#'
...
geom_wnba = function(full_surf = TRUE,
  ...
  # Create the colors to use for the plot
  color_list = wnba_features_set_colors(...)

  # Generate the data frames for the features of a WNBA court
  court_background = wnba_feature_court_bground(full_surf, rotate, rotation_dir)
  center_circles = wnba_feature_center_circles(full_surf, rotate, rotation_dir)
  division_line = wnba_feature_division_line(full_surf, rotate, rotation_dir)
  endline = wnba_feature_endline(full_surf, rotate, rotation_dir)
  sideline = wnba_feature_sideline(full_surf, rotate, rotation_dir)
  court_apron = wnba_feature_court_apron(full_surf, rotate, rotation_dir)
  team_bench = wnba_feature_team_bench(full_surf, rotate, rotation_dir)
  substitution_area = wnba_feature_substitution_area(full_surf, rotate, rotation_dir)
  three_point_line = wnba_feature_three_point_line(full_surf, rotate, rotation_dir)
  free_throw_lane = wnba_feature_free_throw_lane(full_surf, rotate, rotation_dir)
  free_throw_lane_lines = wnba_feature_free_throw_lane_lines(full_surf, rotate)
  free_throw_semi_circle = wnba_feature_free_throw_semi_circle(full_surf, rotate, rotation_dir)
  free_throw_dashed_semi_circle = wnba_feature_free_throw_dashed_semi_circle(full_surf, rotate, rotation_dir)
  restricted_area_arc = wnba_feature_restricted_area_arc(full_surf, rotate, rotation_dir)
  lower_defensive_box = wnba_feature_lower_defensive_box(full_surf, rotate, rotation_dir)
  backboard = wnba_feature_backboard(full_surf, rotate, rotation_dir)
  basket_ring = wnba_feature_basket_ring(full_surf, rotate, rotation_dir)
