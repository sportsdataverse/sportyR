...
#' and helps to optimize performance
#'
...
ncaa_football_feature_yard_markings_df_maker = function(yardage, full_surf = TRUE, rotate = FALSE, rotation_dir = 'ccw'){
  ...
  if(yardage %% 5 == 0){
    if(yardage == 0){
      ...
      yard_marking = data.frame(
        ...
        y = c(
          ...
          -26 - (2/3) + inches_to_yd(4),

          # Lower inbound line marker
          -26 - (2/3) + ft_to_yd(60),
          -26 - (2/3) + ft_to_yd(60),
          -26 - (2/3) + ft_to_yd(60) + inches_to_yd(4),
          -26 - (2/3) + ft_to_yd(60) + inches_to_yd(4),
    ...
    yard_marking_b = create_rectangle(
      ...
      y_min = -26 - (2/3) + inches_to_yd(4),
      y_max = -26 - (2/3) + ft_to_yd(2) + inches_to_yd(4)
    )

    yard_marking_l = create_rectangle(
      x_min = yardage - inches_to_yd(2),
      x_max = yardage + inches_to_yd(2),
      y_min = -26 - (2/3) + ft_to_yd(58),
      y_max = -26 - (2/3) + ft_to_yd(60)
    )

    yard_marking_u = create_rectangle(
      x_min = yardage - inches_to_yd(2),
      x_max = yardage + inches_to_yd(2),
      y_min = 26 + (2/3) - ft_to_yd(60),
      y_max = 26 + (2/3) - ft_to_yd(58)
    )

    yard_marking_t = create_rectangle(
      x_min = yardage - inches_to_yd(2),
      x_max = yardage + inches_to_yd(2),
      y_min = 26 + (2/3) - ft_to_yd(2) - inches_to_yd(4),
      y_max = 26 + (2/3) - inches_to_yd(4)
    )
...
ncaa_football_feature_try_markings = function(full_surf = TRUE, rotate = FALSE, rotation_dir = 'ccw'){
  ...
  try_line = create_rectangle(
    x_min = -47 - inches_to_yd(2),
    x_max = -47 + inches_to_yd(2),
    y_min = -0.5,
    y_max = 0.5
  )
...
#' Generate the data frames for the points that comprise the directional arrows.
#' This function is fed into the ncaa_football_feature_directional_arrows()
#' function below and helps to optimize performance
#'
#' @param yardage The yardage for which to create the yard markings
#' @param full_surf A boolean indicating whether or not to plot a full surface
#'   representation of the surface. Default: \code{TRUE}
  ...
  if(yardage < 0){
    ...
    directional_arrow_1 = data.frame(
      ...
      y = c(
        ...
        -26 - (2/3) + 9 - inches_to_yd(15),
        -26 - (2/3) + 9 - inches_to_yd(15) - inches_to_yd(18),
        -26 - (2/3) + 9 - inches_to_yd(15) - inches_to_yd(9),
        -26 - (2/3) + 9 - inches_to_yd(15)
      )
    )
...
ncaa_football_feature_directional_arrows = function(full_surf = TRUE, rotate = FALSE, rotation_dir = 'ccw'){
  ...
  # result is a list, and will be stacked into a single data frame
  directional_arrows_1_list = lapply(yardages, ncaa_football_feature_directional_arrows_df_maker, full_surf, rotate, rotation_dir, lower = TRUE)
...
#' \code{geom_ncaa_football()} function (or its wrapper \code{geom_football()})
#'
...
#'   colors to pass to the \code{ncaa_football_features_set_colors()} function
#'
...
geom_ncaa_football = function(full_surf = TRUE,
  ...
  # Create the colors to use for the plot
  color_list = ncaa_football_features_set_colors(...)

  # Generate the data frames for the features of an NFL field
  grass = ncaa_football_feature_grass(full_surf, rotate, rotation_dir)
  sideline = ncaa_football_feature_sideline(full_surf, rotate, rotation_dir)
  endline = ncaa_football_feature_endline(full_surf, rotate, rotation_dir)
  goal_line = ncaa_football_feature_goal_line(full_surf, rotate, rotation_dir)
  yard_markings = ncaa_football_feature_yard_markings(full_surf, rotate, rotation_dir)
  try_marking = ncaa_football_feature_try_markings(full_surf, rotate, rotation_dir)
  directional_arrows = ncaa_football_feature_directional_arrows(full_surf, rotate, rotation_dir)
